...
import type {
  ...
  WorkflowGenerationRequestDto,
  WorkflowGenerationResponseDto,
  WorkflowSummaryDto,
...
export async function workflowGenerate(options: WorkflowGenerateOptions): Promise<void> {
  const description = requireString(options.description, '--description');
  const client = createApiClient(options.profile, options.configOptions);

  const requestBody: WorkflowGenerationRequestDto = { description };
  if (options.projectId !== undefined && options.projectId.length > 0) {
    requestBody.projectId = options.projectId;
  }

  const response = await client.post('/workflows/generate', requestBody);
  const result = response.data as WorkflowGenerationResponseDto;
  ...
  if (!result.valid) {
    ...
    log('生成的工作流未通过校验，错误如下：');
    for (const err of result.errors) {
      log(`  [错误] ${err}`);
    }
    if (result.draft) {
      log('最后一次生成的草案：');
      log(JSON.stringify(result.draft, undefined, 2));
    }
    if (options.create) {
      throw new CLIError(
        `生成的工作流未通过校验，无法创建：${result.errors.join('；')}`,
        ErrorCode.ValidationError,
        ExitCode.InvocationError,
      );
    }
  ...
  // --dry-run：草案合法时，调用后端 dry-run 端点验证可运行性。
  // dry-run 失败且 --create 时拒绝创建，形成"生成→修正→再 dry-run"闭环；
  // 否则仅提示（草案仍可保存/创建）。
  if (options.dryRun && result.draft) {
    const draft = (result.draft ?? {}) as Record<string, unknown>;
    const dryRunRequest: DryRunWorkflowRequestDto = {
      nodes: Array.isArray(draft.nodes) ? (draft.nodes as DryRunWorkflowRequestDto['nodes']) : [],
      connections: Array.isArray(draft.connections)
        ? (draft.connections as DryRunWorkflowRequestDto['connections'])
        : [],
      ...(options.credentials && options.credentials.length > 0
        ? { credentials: options.credentials }
        : {}),
    };

    try {
      const drResponse = await client.post('/workflows/dry-run', dryRunRequest);
      const drResult = drResponse.data as ExecutionDto;

      if (isJsonMode()) {
        writeJson({ dryRun: drResult, generation: result });
        return;
      }

      log(`Dry-Run 状态：${drResult.status}`);
      for (const rec of drResult.nodeRecords ?? []) {
        log(`  节点 ${rec.nodeStringId ?? rec.nodeDefinitionId}：${rec.status}`);
      }
    } catch (err) {
      const cliErr =
      ...
      if (options.create) {
        // Dry-Run 失败仍输出 DSL 草案，供用户修正（spec：失败仍输出草案 + 错误）。
        if (result.draft) {
          log('Dry-Run 失败，但生成的工作流草案如下：');
          log(JSON.stringify(result.draft, undefined, 2));
        }
        throw new CLIError(
          `Dry-Run 失败，拒绝创建：${cliErr.message}`,
          ErrorCode.ValidationError,
          ExitCode.InvocationError,
        );
      }
      ...
      if (isJsonMode()) {
        writeJson({
          valid: result.valid,
          draft: result.draft,
          dryRunError: cliErr.message,
          errors: result.errors,
        });
        return;
      }

      log(`Dry-Run 失败（草案仍可保存/创建）：${cliErr.message}`);
      if (result.draft) {
        log('生成的工作流草案如下：');
        log(JSON.stringify(result.draft, undefined, 2));
      }
  ...
  // --create：草案合法时创建工作流（基础版不强制 dry-run）。
  // JSON 模式不交互，直接创建；非 JSON 模式先询问确认。
  if (options.create) {
    const draft = (result.draft ?? {}) as Record<string, unknown>;
    ...
    const dto: CreateWorkflowDto = {
      name,
      createdBy: resolveCreatedBy(config),
      nodes: Array.isArray(draft.nodes) ? (draft.nodes as CreateWorkflowDto['nodes']) : [],
      connections: Array.isArray(draft.connections)
        ? (draft.connections as CreateWorkflowDto['connections'])
        : [],
    };

    if (options.projectId !== undefined && options.projectId.length > 0) {
      dto.projectId = options.projectId;
    } else if (typeof draft.projectId === 'string' && draft.projectId.length > 0) {
      dto.projectId = draft.projectId;
    }

    if (isRecord(draft.styleSettings)) {
      dto.styleSettings = draft.styleSettings as unknown as CreateWorkflowDto['styleSettings'];
    }

    if (!isJsonMode()) {
      const confirmed = await confirmPrompt('是否基于该草案创建工作流？');
      if (!confirmed) {
        log('已取消创建。');
        return;
      }
    ...
    const createResponse = await client.post('/workflows', dto);
    const workflow = createResponse.data as WorkflowDto;
  ...
  if (result.draft) {
    log(`已生成工作流草案（尝试 ${result.attempts} 次）：`);
    log(JSON.stringify(result.draft, undefined, 2));
  }
  ...
  if (options.output !== undefined && options.output.length > 0) {
    const { writeFileSync } = await import('node:fs');
    try {
      writeFileSync(options.output, JSON.stringify(result.draft, undefined, 2), 'utf-8');
    } catch (err) {
      const message = err instanceof Error ? err.message : String(err);
      throw new CLIError(
        `写入文件失败：${message}`,
    ...
    log(`已保存到：${options.output}`);
  }

  log('提示：使用 --create 可直接创建，或 --output 保存草案。');
}
