...
describe('commands/workflows', () => {
  ...
  describe('workflow generate', () => {
    const validDraft = {
      ...
      nodes: [{ id: 'n1', typeName: 'manualTrigger', name: '开始', isEntry: true }],
      connections: [],
    };

    it('returns draft in JSON mode', async () => {
      mockInstance.post.mockResolvedValue({
        data: { valid: true, draft: validDraft, errors: [], attempts: 1 },
      });
      ...
      expect(parsed.draft.name).toBe('Generated Workflow');
      expect(mockInstance.post).toHaveBeenCalledWith('/workflows/generate', {
        description: '生成一个工作流',
      });
    ...
    it('writes draft to output file', async () => {
      mockInstance.post.mockResolvedValue({
        data: { valid: true, draft: validDraft, errors: [], attempts: 1 },
      });

      const outputPath = join(tempDir, 'generated.json');
      await workflowGenerate({
        description: '生成一个工作流',
        output: outputPath,
        configOptions: options,
      ...
      expect(written.name).toBe('Generated Workflow');
      expect(mockInstance.post).toHaveBeenCalledOnce();
    });

    it('creates workflow in JSON mode when --create is set', async () => {
      mockInstance.post.mockResolvedValueOnce({
        data: { valid: true, draft: validDraft, errors: [], attempts: 1 },
      });
      mockInstance.post.mockResolvedValueOnce({
        data: { id: 'wf-gen', name: 'Generated Workflow', version: 1, isActive: true, createdBy: 'user-1', createdAt: '2025-01-01T00:00:00Z', nodes: [], connections: [] },
      });
      ...
      expect(mockInstance.post).toHaveBeenNthCalledWith(1, '/workflows/generate', {
        description: '生成一个工作流',
      });
      expect(mockInstance.post).toHaveBeenNthCalledWith(2, '/workflows', {
        name: 'Generated Workflow',
        createdBy: 'user-1',
        nodes: validDraft.nodes,
        connections: [],
      });
    ...
    it('does not create and throws when draft is invalid', async () => {
      mockInstance.post.mockResolvedValue({
        data: { valid: false, draft: null, errors: ['节点使用了未知类型'], attempts: 3 },
      });
    ...
    it('calls dry-run endpoint when --dry-run is set', async () => {
      mockInstance.post.mockResolvedValueOnce({
        data: { valid: true, draft: validDraft, errors: [], attempts: 1 },
      });
      mockInstance.post.mockResolvedValueOnce({
        data: {
          ...
          nodeRecords: [
            { id: 'r1', nodeDefinitionId: 'n1', runIndex: 0, status: 'Completed' },
          ],
        },
      });
      ...
      expect(mockInstance.post).toHaveBeenNthCalledWith(2, '/workflows/dry-run', {
        nodes: validDraft.nodes,
        connections: [],
      });
      expect(output).toContain('Dry-Run 状态：DryRunCompleted');
      expect(output).toContain('n1：Completed');
    });

    it('refuses to create when --dry-run fails and --create is set', async () => {
      mockInstance.post.mockResolvedValueOnce({
        data: { valid: true, draft: validDraft, errors: [], attempts: 1 },
      });
      mockInstance.post.mockRejectedValueOnce(
        Object.assign(new Error('node failed'), { response: { status: 500, data: { message: '执行失败' } } }),
      );
