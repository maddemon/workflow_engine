...
/// 节点自身不构建循环。</para>
/// </summary>
public sealed class BatchSplitNode : INodeType
{
    /// <inheritdoc />
    public string TypeName => "batchSplit";

    /// <inheritdoc />
    public string DisplayName => "Split In Batches";
    ...
    public Task<NodeExecutionResult> ExecuteAsync(NodeExecutionContext context, CancellationToken ct = default)
    {
        ...
        BatchLoopHelper.EnsureInitialized(nodeContext, context.GetInputBatch().Items);
