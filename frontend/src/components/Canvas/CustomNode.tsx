...
const AI_INPUT_SORT_ORDER: Record<string, number> = { LLM: 0, Memory: 1, AgentTool: 2 }
...
function CustomNodeComponent({ id, data, selected }: NodeProps<WorkflowNode>) {
  ...
  const nodeWidth = circular
    ? 56
    : configurable
      ? Math.max(160, outputPorts.length * 32)
  ...
  return (
    <div className={nodeClasses} style={{ width: nodeWidth, height: nodeHeight }}>
      <div
        ...
        {badgeColor && (
          <div className="node-badge" style={{ background: badgeColor }}>
            {data.isEntry && (!status || status === "idle") && (
              <Play size={7} color="var(--mantine-color-white)" fill="var(--mantine-color-white)" />
            )}
      ...
      {visibleInputPorts.map((port) => {
        ...
        const posClass =
          layout.position === Position.Bottom
            ? "port-bottom"
            : layout.position === Position.Top
              ? "port-top"
              : layout.position === Position.Left
                ? "port-left"
                : "port-right"
        return (
          <Handle
