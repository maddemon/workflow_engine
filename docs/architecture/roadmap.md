...
增加企业基础能力：权限、项目分类、安全、文件、Agent 增强。

### 功能清单

| 模块 | 功能点 |
|------|--------|
| **限流** | IP 限流（登录）、用户限流（API 调用）、配置化阈值 |
| **权限 RBAC** | 角色定义（管理员/编辑/查看）、Scope 枚举、中间件鉴权 |
| **项目分类** | 项目 CRUD、资源按项目分组、ProjectId 分类字段与筛选 |
...
- Beta 的 RBAC/项目分类依赖 Alpha 的用户系统。
...
| 项目分类误用为隔离条件 | 数据被错误隐藏 | 代码审查明确“仅筛选、不隔离” |

## 10. 变更记录

| 日期 | 修改人 | 修改内容 | 关联任务 |
|------|--------|----------|----------|
| 2026-07-04 | Agent | 将多租户/项目调整为项目分类，弱化隔离描述 | task-align-no-saas-multitenant |
